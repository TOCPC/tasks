...
class disjoint_set {
    ...
    vector<int> parent;
    public:
    disjoint_set(int n) : size(n), parent(n), merge_count(0) {
        for (int i = 0; i < n; ++i)
            parent[i] = i;
    }
    int count_component() {
...
/**
 * Generate edge list of random directed acyclic graph (DAG) with n vertices and m edges
 * Vertices are indexed from 0 to n-1
 */
vector<pii> gen_DAG(int n, int m)
{
    vector<pii> E = gen_undirected_graph(n, m);
    vector<int> val(n);
    for (int i = 0; i < n; ++i)
        val[i] = i;
    shuffle(val.begin(), val.end());
    for (auto &e : E) {
        if (val[e.second] < val[e.first])
...
/**
 * Generate edge list of bipartite graphs consisting of n1 nodes on left side, n2 nodes on right side and m edges
 * Vertices are indexed from 0 to n-1
 */
vector<pii> gen_bipartite_graph(int n1, int n2, int m)
{
    int n = n1+n2;
    ensure(m >= 0 && m <= n1*1LL*n2);
    vector<ll> s = pick_set(n1*1LL*n2, m);
    shuffle(s.begin(), s.end());
    vector<pii> edges(m);
    vector<int> idx(n);
    for (int i = 0; i < n; ++i)
        idx[i] = i;
    for (int i = 0; i < m; ++i) {
        edges[i] = pii(idx[s[i]/n2], idx[n1+s[i]%n2]);
        if (rnd.next(2))
            swap(edges[i].first, edges[i].second);
    }
    shuffle(edges.begin(), edges.end());
...
vector<pii> gen_con_directed_graph(int n, int m, int s, int t, int k=-1)
{
    if (s == t)
        return gen_directed_graph(n, m);

    ensure(k <= n-2); // k = internal nodes in path
    if (k < 0)
        k = rnd.next(0, n-2);
    vector<int> path = pick_set(n, k+2), npath; // picked 2 more just in case it collides with s or t 
    for (auto x : path) {
        if (x != s && x != t)
            npath.push_back(x);
    }
    shuffle(npath.begin(), npath.end());
    npath.resize(k);
    ...
    vector<ll> sn = pick_set(n*1ll*(n-1), min<ll>(m+k, n*1ll*(n-1)));
    shuffle(sn.begin(), sn.end());
    for (int i = 0; edges.size() < m; ++i) {
        pii e = get_directed_edge(n, m, sn[i]);
        if (!edges.count(e))
            edges.insert(e);
    }

    vector<pii> ret(edges.begin(), edges.end());
    shuffle(ret.begin(), ret.end());
    return ret;
}
