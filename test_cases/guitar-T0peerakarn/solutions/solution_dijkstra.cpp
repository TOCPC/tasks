...
int main()
{
    ...
    for(int i=0 ; i<=m+1 ; i++) for(int j=0 ; j<=k ; j++) dis[i][j] = 1e18;
    dis[0][k] = 0;
    pq.push({0, 0, k});
    while(!pq.empty())
    {
        ...
        for(int i=0 ; i<=nows && nown+i+1<=m+1 ; i++) if(dis[nown+i+1][nows-i] > noww + p[s[nown]][s[nown+i+1]])
        {
